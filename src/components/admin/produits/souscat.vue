...
computed:{
 ...
 totalPages() {
 return Math.ceil(this.SousCategorieOptions.length / this.itemsPerPage);
 },
 paginatedItems() {
   const startIndex = (this.currentPage - 1) * this.itemsPerPage;
   const endIndex = startIndex + this.itemsPerPage;
   return this.SousCategorieOptions.slice(startIndex, endIndex);
 },
},
async mounted() {
  ...
  if (this.v$.$errors.length == 0 ) {
     this.loading = true
       let DataUser = {
        ...
        CategorieProduitsId:this.step1.categorie,
       ...
       if (response.data.status === "success") { 
         this.AddUser = false
         this.loading = false
         this.successmsg("Création de sous-categorie produit",'Votre sous-categorie produit a été crée avec succès !')
        await this.fetchSousCategorieProduits()
   ...
   if (result.isConfirmed) {
     this.DeleteUser(id);
       ...
       try {
         ...
         const response = await axios.delete(`/sous-produits/${id}`, {
           headers: {
             Authorization: `Bearer ${this.loggedInUser.token}`,
         ...
         await this.fetchSousCategorieProduits()
           this.loading = false
          this.successmsg('Supprimé!', 'Votre sous-categorie produit a été supprimée.')
     ...
     async UpdateUser(id) {
       ...
       try {
           ...
           const user = this.SousCategorieOptions.find(user => user.id === id);

           if (user) {
               // Utilisez les informations récupérées de l'objet user
               console.log('Informations de l\'utilisateur:', user);
                    this.step2.nom = user.NomCategorieProduit,
                    this.step2.categorie =  user.CategorieProduitsId
                    this.ToId = user.id
           } else {
    ...
    if (this.v$.$errors.length == 0) {
      console.log("bonjour");
       this.loading = true;
          ...
          CategorieProduitsId:this.step2.categorie,
           id:this.ToId
           }
   console.log('dataCath',dataCath);
 
      try {
        const response = await axios.post(`sous-produits/${this.ToId}/update`,dataCath, {
          headers: {
        ...
        if (response.data.status === "success") {
         ...
         this.loading = false
         this.successmsg("Modification de sous-categorie produit",'Votre sous-categorie produit a été modifiée avec succès !')
