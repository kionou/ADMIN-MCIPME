      ...
      <BCard no-body>
        <BCardBody class="border-bottom">
          <div class="d-flex align-items-center justify-content-between">
            <BCardTitle class="mb-0 ">Liste des sous secteurs</BCardTitle>
            ...
            <BTableSimple class="align-middle table-nowrap table-hover">
              <BThead class="table-light" style="">
                <BTr>
                  ...
                  <BTh scope="col">Nom sous secteur</BTh>
                  <BTh scope="col">Secteur d'activité</BTh>
              ...
              <BTbody>
                <BTr v-for="region in paginatedItems" :key="region.id">
                  ...
                  <BTd>
                   ...
                   {{ region.CodeSousecteur }}
                  </BTd>
                  <BTd>{{ region.NomSousSecteur }}</BTd>
                  <BTd>
                   
                   {{ NameRegion(region.CodeSecteurActivite)  }}
                  </BTd>
                  ...
                  <BTd>
                    <ul class="list-unstyled hstack gap-1 mb-0">
                      ...
                      <li data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Delete">
                        <Blink href="#" @click="confirmDelete(region.CodeSousecteur)" data-bs-toggle="modal" class="btn btn-sm btn-danger"><i class="mdi mdi-delete-outline"></i></Blink>
                      </li>
              ...
              <BRow>
                <BCol cols="12 text-center">
                  <div class="modalheader p-4">
                    <h5 class="text-primary">Ajouter un sous secteur</h5>
                ...
                <BForm class="form-horizontal">
                   <BRow>
                    ...
                    <div class="mb-3 position-relative">
                      <label for="userpassword">Secteur d'activité</label>
                      <MazSelect label="Sélectionner le secteur d'activité" v-model="step1.secteur" color="info" :options="SecteurActiviteOptions" search />
                     <small v-if="v$.step1.secteur.$error">{{v$.step1.secteur.$errors[0].$message}}</small> 
              ...
              <div class="p-2">
                <BForm class="form-horizontal">
                 <BRow>
                    ...
                    <div class="mb-3 position-relative">
                      <label for="userpassword">Secteur d'activité</label>
                      <MazSelect label="Sélectionner le secteur d'activité" v-model="step2.secteur" color="info" :options="SecteurActiviteOptions" search />
                     <small v-if="v$.step2.secteur.$error">{{v$.step2.secteur.$errors[0].$message}}</small> 
                     <small v-if="resultError['CodeSecteurActivite']"> {{ resultError["CodeSecteurActivite"] }} </small>
...
export default {
 ...
 computed:{
   ...
   totalPages() {
   return Math.ceil(this.SousSecteurActiviteOptions.length / this.itemsPerPage);
   },
   paginatedItems() {
     const startIndex = (this.currentPage - 1) * this.itemsPerPage;
     const endIndex = startIndex + this.itemsPerPage;
     return this.SousSecteurActiviteOptions.slice(startIndex, endIndex);
   },
 },
async mounted() {
   console.log("uusers",this.loggedInUser);
  await this.fetchSecteurActiviteOptions()
  await this.fetchSousSecteurOptions()
 ...
 methods: {
   ...
   async fetchSecteurActiviteOptions() {
    try {
              ...
              params: {Direction: this.loggedInUser.direction,},
               ...
               this.SecteurActiviteOptions =response.data.data.data.map(sousprefecture => ({
                    label: sousprefecture.NomSecteurActivite,
                    value: sousprefecture.CodeSecteurActivite,
                    code:sousprefecture.CodeSecteurActivite
                }));
    ...
    try {
      ...
      this.SousSecteurActiviteOptions1 = []
      this.SousSecteurActiviteOptions = []


      // this.SousSecteurActiviteOptions = response.data.data
        response.data.data.data.forEach(secteur => {
            secteur.sous_secteurs.forEach(sousSecteur => {

              this.SousSecteurActiviteOptions1.push(sousSecteur); // Ajout de chaque sous-secteur à la liste globale
            });
          });
          this.SousSecteurActiviteOptions = this.SousSecteurActiviteOptions1
      console.log('soussecteur22',  this.SousSecteurActiviteOptions);

      this.loading = false
      // Appeler la mutation pour mettre à jour les options de sous-secteurs
    } catch (error) {
   ...
   async HamdleAddUser(){
    ...
    if (this.v$.$errors.length == 0 ) {
       this.loading = true
         let DataUser = {
            CodeSousecteur:this.step1.code,
            NomSousSecteur:this.step1.nom,
            CodeSecteurActivite:this.step1.secteur,
            Direction:this.loggedInUser.direction
         }
         console.log("eeeee",DataUser);
         try {
        
         const response = await axios.post('/sous-secteurs' , DataUser, {
             headers: {
         ...
         if (response.data.status === "success") { 
          await this.fetchSousSecteurOptions()
           this.AddUser = false
           this.loading = false
           this.successmsg("Création du sous secteur d'activité","Votre  Sous secteur d'activité a été crée avec succès !")
     ...
     if (result.isConfirmed) {
       this.DeleteUser(id);
         ...
         try {
           ...
           const response = await axios.delete(`/sous-secteurs/${id}`, {
             headers: {
               Authorization: `Bearer ${this.loggedInUser.token}`,
           ...
           if (response.data.status === 'success') {
             this.loading = false
            this.successmsg('Supprimé!', 'Votre sous secteur a été supprimée.')
          await this.fetchSousSecteurOptions()
       ...
       async UpdateUser(id) {
         ...
         try {
             ...
             const user = this.SousSecteurActiviteOptions.find(user => user.id === id);

             if (user) {
                 // Utilisez les informations récupérées de l'objet user
                 console.log('Informations de l\'utilisateur:', user);

                this.step2.code = user.CodeSousecteur,
                this.step2.nom = user.NomSousSecteur,
                this.step2.secteur = user.CodeSecteurActivite,
                this.ToId = user.CodeSousecteur
             } else {
   ...
   async  submitUpdate(){
        ...
        console.log("bonjour");
         this.loading = true;
               ...
               const dataCath = {
                ...
                CodeSousecteur:this.step2.code,
                NomSousSecteur:this.step2.nom,
                CodeSecteurActivite:this.step2.secteur,
                StatutSousSecteur:1,
                Direction:this.loggedInUser.direction
     ...
     console.log('dataCath',dataCath);
        ...
        try {
          const response = await axios.put(`/sous-secteurs/${this.ToId}`,dataCath, {
            headers: {
          ...
          if (response.data.status === "success") {
           ...
           this.loading = false
           this.successmsg("Modification du sous secteur d'activité","Votre sous secteur d'activité a été modifiée avec succès !")
