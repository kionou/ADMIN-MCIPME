               ...
               <BRow>
                 <BCol cols="12 text-center">
                   <div class="modalheader p-4">
                     <h5 class="text-primary">Modifier un partenaire</h5>
  ...
  methods: {
   ...
   async SubmitPartenaire() {
     ...
     if (this.v$.$errors.length == 0) {
       ...
       try {
         const response = await axios.post(`/partenaires/${this.id}`, formData, {
           headers: {
             "Content-Type": "multipart/form-data",
             Authorization: `Bearer ${this.loggedInUser.token}`,
           },
         ...
         if (response.data.status === "success") {
           this.loading = false
           this.successmsg("Modification d'un partenaire",'Votre partenaire a été modifié avec succès !')
