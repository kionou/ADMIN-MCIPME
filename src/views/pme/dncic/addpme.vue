<template >
   ...
   <PageHeader title="Ajouter entreprise" pageTitle="Entreprise" />
   <BRow>
     <BCol lg="12">
       <BCard no-body>
...
async mounted() {
   ...
   const localStorageUserData = localStorage.getItem('tempMpmeDataDNCIC') || null;
...
async nextStep() {
            ...
            if( this.currentStep === 6){
               ...
               const stepData = this.collectStepData(this.currentStep);
                console.log(stepData);
                  ...
                  const stepData = this.collectStepData(this.currentStep);
                  console.log(stepData);
                  localStorage.setItem('tempMpmeDataDNCIC', JSON.stringify(stepData))
                  const response = await this.updateMpmeDonnees(stepData) 
                  console.log('eee',response);
                  if (response === true) {
                      this.currentStep++;
    ...
    //       localStorage.setItem('tempMpmeDataDNCIC', JSON.stringify(mpmeData));
    //       this.currentStep++;
    //       window.scrollTo({
    //           top: 0,
    //           behavior: "smooth",
    ...
    //       if (success) {
    //           localStorage.removeItem('tempMpmeDataDNCIC')
    //          setTimeout(()=>{
   ...
   if (result.isConfirmed) {
    ...
    localStorage.removeItem('codempmeDNCIC')
    localStorage.removeItem('tempMpmeDataDNCIC')
        this.valider()
    ...
    try {
        ...
        if (response.data.status === 'success') {
            console.log("Données MPME mises à jour avec succès !");
            localStorage.setItem('codempmeDNCIC', response.data.data.user.Entreprises);
            return true;
        } else {
            console.error("Erreur lors de la mise à ", response.data);
...
async updateMpmeDonnees(mpmeData) {
    try {
      const CodeMpme = localStorage.getItem('codempmeDNCIC');
      console.log('codempmeDNCIC',CodeMpme)
;
